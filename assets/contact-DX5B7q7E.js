import{r as d,a as m,$ as a,P as o,c as i,S as s,i as x,b as u,t as r}from"./utils-CX3gqYKq.js";import{s as b}from"./api-1QFzz9sZ.js";const p="/";function g(){d("Contact"),m();const n=a("#content");if(!n)return;n.innerHTML=`
    <section class="max-w-4xl mx-auto px-4 sm:px-6 py-12 md:py-16">
      <div class="text-center mb-12 fade-in">
        <h1 data-cms="contact.hero.heading" class="text-3xl md:text-5xl font-light text-bark">${o.contact.heroHeading}</h1>
        <p data-cms="contact.intro" class="text-bark-50 mt-3">${o.contact.intro}</p>
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-10">
        ...
        <div class="space-y-8 fade-in">
          <div class="flex items-start gap-4">
            <span class="text-sage mt-0.5">${i.mail}</span>
            <div>
              <h3 class="font-medium text-bark text-sm">Email</h3>
              <p class="text-bark-50 text-sm">${s.email}</p>
            </div>
          </div>

          <div class="flex items-start gap-4">
            <span class="text-sage mt-0.5">${i.instagram}</span>
            <div>
              <h3 class="font-medium text-bark text-sm">Instagram</h3>
              <p class="text-bark-50 text-sm">${s.instagram}</p>
            </div>
          </div>
          ...
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <p class="text-bark-50 text-sm">${s.location}</p>
            </div>
          </div>
  ...
  `,x(),u();const e=a("#contact-form");e.addEventListener("submit",async l=>{l.preventDefault();const t=a("#contact-btn");t.disabled=!0,t.textContent="Sending...";try{await b({name:e.name.value.trim(),email:e.email.value.trim(),details:"[Contact Form Message] "+e.message.value.trim(),phone:"",delivery:"inquiry"}),e.classList.add("hidden"),a("#contact-success").classList.remove("hidden"),r("Message sent!")}catch(c){r(c.message||"Failed to send. Please try again.","error"),t.disabled=!1,t.textContent="Send Message"}})}document.addEventListener("DOMContentLoaded",g);
