...
This should be set up under the bakery's own Google account (not the
developer's) — the Sheet, the sent emails, and the deployment all belong to
whoever runs `Deploy`.
...
If you see that, the deployment worked. Send the `/exec` URL back to the
developer — it needs to be set as the `VITE_APPS_SCRIPT_URL` GitHub Actions
secret (for the live site) and in a local `.env` file (for local dev/testing).
